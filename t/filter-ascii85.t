...
is(PDF::Core::Filter::ASCII85.encode($in),
   $out,
   q{ASCII85 test string is encoded correctly});

is(PDF::Core::Filter::ASCII85.decode($out),
   $in,
   q{ASCII85 test string is decoded correctly});

dies_ok { PDF::Core::Filter::ASCII85.decode($out, :eod) },
...
is(PDF::Core::Filter::ASCII85.encode($in, :eod),
   $out,
...
dies_ok { PDF::Core::Filter::ASCII85.decode('This is not valid input{|}') },
