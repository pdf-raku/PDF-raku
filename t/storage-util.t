...
is-deeply $result=resample($bytes,  8, 4), (0, 10, 1, 4, 1, 14, 2, 8, 3, 2, 3, 12).list.item, '4 bit resample';
is-deeply resample($result, 4, 8), $bytes, 'resample round-trip: 8 => 4 => 8';

is-deeply resample($bytes,  8, 8), $bytes, '8 bit resample';

is-deeply $result=resample($bytes,  8, 16), (2580, 7720, 12860).list.item, '16 bit resample';
is-deeply resample($result, 16, 8), $bytes, 'resample round-trip: 16 => 8 => 16';

is-deeply $result=resample($bytes,  8, 6), (2, 33, 16, 30, 10, 3, 8, 60).list.item, '6 bit resample';
