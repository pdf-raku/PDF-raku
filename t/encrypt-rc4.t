...
lives-ok { $pdf = PDF.open("t/encrypt.pdf", :password($user-pass)) }, 'open with user password - lives';
...
lives-ok { $pdf = PDF.open("t/encrypt.pdf", :password($owner-pass)) }, 'open with owner password - lives';
...
dies-ok { $pdf = PDF.open: "t/encrypt.json", :password<dunno> }, "open encrypted json with incorrect password - dies";

lives-ok { $pdf = PDF.open("t/encrypt.json", :password($user-pass)) }, 'open json user password - lives';
