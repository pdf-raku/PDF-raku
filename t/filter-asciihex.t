...
is(PDF::Core::Filter::ASCIIHex.encode($in),
   $out,
   q{ASCIIHex test string is encoded correctly});

is(PDF::Core::Filter::ASCIIHex.decode($out),
   $in,
   q{ASCIIHex test string is decoded correctly});

dies_ok { PDF::Core::Filter::ASCIIHex.decode($out, :eod) },
...
is(PDF::Core::Filter::ASCIIHex.encode($in, :eod),
   $out,
...
dies_ok { PDF::Core::Filter::ASCIIHex.decode('This is not valid input') },
...
my $actual_bytes = PDF::Core::Filter::ASCIIHex.decode($odd_out).comb>>.ord.join: ' ';
is($actual_bytes,
   $expected_bytes,
