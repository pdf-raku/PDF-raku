...
my $ind-obj = PDF::Basic::IndObj.indobj-new( :$dict );
isa_ok $ind-obj, PDF::Basic::IndObj::XRef;

my $xref;
lives_ok { $xref = $ind-obj.decode( $raw-content, :$dict ) }, 'basic content decode - lives';
...
my $xref-recompressed = $ind-obj.encode( $xref, :$dict);
my $xref-roundtrip = $ind-obj.decode( $xref-recompressed, :$dict);
