...
# See [PDF 1.7 TABLE 3.18 Entries common to all encryption dictionaries]
   ...
   has PDF::Object::Name $.SubFilter is entry;             #| Optional; PDF 1.3) A name that completely specifies the format and interpretation of the contents of the encryption dictionary. It is needed to allow security handlers other than the one specified by Filter to decrypt the document. If this entry is absent, other security handlers should not be allowed to decrypt the document.
    ...
    has PDF::Object::Name $.StmF is entry;                 #| (Optional; meaningful only when the value of V is 4; PDF 1.5) The name of the crypt filter that is used by default when decrypting streams. The name must be a key in the CF dictionary or a standard crypt filter name specified in Table 3.23. All streams in the document, except for cross-reference streams (see Section 3.4.7, “Cross-Reference Streams”) or streams that have a Crypt entry in their Filter array (see Table 3.5), are decrypted by the security handler, using this crypt filter.
                                                           #| Default value: Identity.

    has PDF::Object::Name $.StrF is entry;                 #| (Optional; meaningful only when the value of V is 4; PDF 1.5) The name of the crypt filter that is used when decrypting all strings in the document. The name must be a key in the CF dictionary or a standard crypt filter name specified in Table 3.23.
                                                           #| Default value: Identity.

    has PDF::Object::Name $.EFF is entry;                  #| (Optional; meaningful only when the value of V is 4; PDF 1.6) The name of the crypt filter that should be used by default when encrypting embedded file streams; it must correspond to a key in the CF dictionary or a standard crypt filter name specified in Table 3.23.
   ...
   # See [PDF 1.7 TABLE 3.19 Additional encryption dictionary entries for the standard security handler]
    ...
    has UInt $.P is entry;      #| (Required) A set of flags specifying which operations are permitted when the document is opened with user access
